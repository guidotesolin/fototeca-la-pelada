...
| F42     | A translation written straight into Postgres can take a day to appear                                                                                          | The public reads are `unstable_cache` with a day's floor, invalidated by `revalidateTag(GALLERY_TAG)` — which the panel calls on every write and a `psql` session does not. Found while verifying T13: the site went on serving Spanish for a caption that was already English in the database, which reads exactly like a leaking cache key and is not one. It matters because T15 loads the translations and has no editor to load them through yet. Either give it one, or have it end with a revalidation                                                                                                                                                                                               | T15                                                           |
...
| F45     | The panel can say what is untranslated but not translate it                                                                                                    | `/admin/translations` is read-only: T13's card scopes it to "a panel view of what is untranslated per language", and the write path is a per-language form on the photo, section and site-text screens — three screens' worth of editing, which is a task and not a corner of this one. Until it exists the translations go into the database directly, which is F42's problem too                                                                                                                                                                                                                                                                                                                          | T15                                                           |
