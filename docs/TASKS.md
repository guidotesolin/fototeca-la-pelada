...
**Both halves are done.** The platform half was executed by the maintainer with the session
verifying each step from outside: the domain at NIC delegated to Cloudflare, Vercel connected to
`main` with the twelve variables (`AUTH_URL` deliberately absent), the apex serving with `www`
redirecting to it, `img.fototecalapelada.com.ar` as the bucket's custom domain with the development
URL off and `masters/` blocked at the edge, the production redirect URI on the existing OAuth client,
and a Search Console domain property owned by the archive's own account. _Production_ in ARCHITECTURE
carries what was decided and why; the measurements are below.

**Verified against the live domain, not against a laptop**: six security headers on every response
including one served `x-vercel-cache: HIT`, the `/admin` variant of the CSP only on `/admin`, 2,492
sitemap entries over 592 published photographs, 429 on the 31st search in a minute, 410 in all four
languages plus the percent-encoded spelling on the first request, and the masters prefix answering 403
to four spellings while the renditions answer 200.
...
The three YouTube interviews **F5** recorded get a section of their own, `/videoteca`, with a
shareable page per interview. They are interviews with neighbours, from the archive's own channel,
and the old Google Sites announced each one at the foot of its section -- which is the failure the
whole move was made for: nothing had an address of its own.

Three decisions were taken with the maintainer before any markup:

- **The home page**: not in the Swiper deck, but a card in the `#secciones` grid and **always
  last**, so `category.position` -- which the panel edits -- is untouched. The deck and the heading
  above that grid both count photographs, so a twelfth card holding none would make the sentence
  untrue; this one counts interviews and says so.
- **The titles**: `Memorias de La Pelada — María Luisa Ravasio`, `— Sunilda Rizzo`, `— Manuel
Díaz`. The series name is how the old site titled all three, in the captions of `espacios-070`,
  `trabajo-055` and `eucaliptus-043`. Nothing was invented: the names are in `archive.json`, and
  YouTube's own titles are the poorer source -- two of the three carry no surname.
- **The slugs**: the archive's own convention, `videoteca-001` to `-003`.

**Navigation was left to the session and went inside the existing _Secciones_ menu**, as a last
entry after the eleven with a rule above it. Not a second word in the header bar: at 375 px the row
is already the mark, the hamburger, the search field and the settings, and `MenuDismiss` covers any
`details.menu` for free.

What it is made of, and the four decisions worth keeping:

- **`video` + `video_translation`, and no master.** _Data model_ carries the shape. Principle 1 says
  the master is the document, and this document is the video, which lives on YouTube: what is stored
  is the poster, so there is no `master_source` and `readMaster` never enters this path. The four
  derivative columns carry `photo`'s own names, which is what lets a row satisfy `PhotoCard` and be
  drawn by `PhotoImage` with no component of its own.
- **The id is stored and a URL never is.** `videoEmbedUrl` builds the address, so nothing an
  administrator types reaches an `<iframe src>` as an address. `VIDEO_HOSTS` in `src/lib/url.ts` is
  exported for the reason `MAP_HOSTS` is: `frame-src` is built from both, so the allowlist and the
  header cannot become two versions of one fact.
- **The poster is downloaded once and served from R2**, which is what keeps `img-src` unchanged and
  what makes the Open Graph card possible. Measured on the production build, reading the network log
  rather than the code: a loaded ficha makes **zero** requests to any Google-controlled domain, and
  `youtube-nocookie.com` appears on the click and not before.
- **Facade, and it degrades to a link.** The player is created by a 30-line client component on the
  click; with no JavaScript the poster sits inside a real anchor to YouTube, and the server HTML
  carries no iframe at all. A CSS-only version does not exist -- a hidden `<iframe>` still loads its
  `src`, and a server-rendered `?ver=1` reads `searchParams`, which would make the page dynamic.

**The letterbox, which was a defect before it was a decision.** `hqdefault` is 480×360 and pads a
16:9 video with 45 px bars, so the first version drew the one card on the home page with black
edges -- seen on the running build, beside eleven photographs. The poster is now cropped to its
central 16:9 at import, and that crop is **YouTube's own geometry rather than an estimate**: it is
the frame YouTube itself publishes as `mqdefault`, which the two agree on to a mean of 4-7 of 255
per pixel. `sharp.trim()` was the other candidate and was rejected -- it needs a threshold, and on
these three it landed anywhere between 1.69 and 1.75. For a 16:9 source the same arithmetic is the
identity, so there is no branch.

**And it took a schema correction to make the i18n it promises actually possible.** `title` shipped
`NOT NULL`, copied from `category_translation.name`, so `writeTranslations` refused a description
whose title was blank. That is the state all three interviews are in: the title is a series plus a
person's name and reads the same in four languages, while the description is prose that does not.
Translating a description would have meant inventing a translation of a proper noun, which the
glossary exists to prevent. Found by running `translations:load`, which refused all three; `0008`
drops the constraint and `writeVideos` became the per-field shape of `writePhotos`.

Everything else rides what already exists rather than adding a second path: the 410 is `proxy.ts`'s,
with `listGoneSlugs` becoming `listGonePaths` so a hidden video and a hidden photograph are one list,
one memo and one fetch; the sitemap is `listPublicPaths`; the translation boxes are T15's, inside the
form and the transaction the screen already had; and `TakedownHelp` now takes a path, so a hidden
interview gets the same Search Console runbook.

_Acceptance_: the three interviews list and each has its own address in four languages; the ficha is
shareable, with a canonical, four `hreflang` plus `x-default`, and an `og:image` from R2; with
JavaScript disabled the poster is inside a working link to YouTube and the HTML holds no iframe;
**measured on the network log, the page asks nothing of Google until the play button is pressed**;
`frame-src` carries YouTube and `img-src` is unchanged; unpublishing drops it from the list, the home
card and the sitemap and answers 410 in all four languages, encoded spellings included, and
republishing reuses the same poster key; a description translates with its title left in Spanish and
...
| ~~F5~~  | ~~The three YouTube interviews in `archive.json` have no home in the data model~~                                                                              | One video in three sections is not a schema, and nothing in the design consumes them yet. **Closed in T16**, and the answer was neither "at the foot of their section" nor a column: they are a **Videoteca**, a section of its own at `/videoteca` with a shareable address per interview, on the same criterion the whole archive was moved for. A `video` + `video_translation` pair beside `photo`, storing the id rather than a URL so the embed cannot be pointed anywhere, and a poster downloaded once into R2 so the page asks Google for nothing until somebody presses play. The three ids in `archive.json` are rows now; `sections[].videos` is what they were rescued from and is not read by anything                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | T16                                                        |
...
| F10     | A takedown is verified through `HeadObject`, not through a public URL                                                                                          | **Now doable, and still not done.** `img.fototecalapelada.com.ar` exists as of T14 and `masters/` is blocked at the edge, so there is finally a public URL whose answer means something. `tools/takedown-smoke.ts` still asks R2 directly, which tests the bucket rather than the promise.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Whoever touches the takedown path next                     |
...
| ~~F13~~ | ~~`/creditos` appears in the repository layout but no card builds it~~                                                                                         | T6 covered the layout, the home page and the galleries, which is what its card scoped. **`/sobre` came out of this row in T14 and is not being built** -- see _Exposure, indexing and takedown on request_ for where the contact procedure lives instead. **`/creditos` is dropped too, and that is a decision rather than a deferral**: nothing links it either, and the two things it would carry -- the rights notice and the per-photo credit that _Risks_ calls a requirement -- are already in the footer and on every photograph, so the page would be a third address for text the reader has been shown already. It comes out of the repository layout in `ARCHITECTURE.md` with `/sobre`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | done                                                       |
...
| ~~F16~~ | ~~`NEXT_PUBLIC_IMAGE_BASE_URL` points at R2's development URL (`pub-….r2.dev`)~~                                                                               | **Closed in T14.** `img.fototecalapelada.com.ar` is the bucket's custom domain and the Public Development URL is disabled -- `pub-….r2.dev` now answers 401 for `photos/` and `masters/` alike, so there is one public way to a rendition and it is the one in the CSP. The CSP followed on its own, since `img-src` is built from this variable. `.env.local` moved with it, or local development would have lost its images.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | T14                                                        |
...
| F20     | The gallery's reading order is column-wise, not left to right                                                                                                  | Inherent to CSS multi-column, and the same property the Europeana reference has. Recovering left-to-right order **and** tight packing needs JavaScript or `grid-template-rows: masonry`. **Measured on the live site, 1280 px, `campo`, 24 photographs**: the wall is 2195 px tall and the second cell sits _below_ the first, so the order is column-wise as the row says. `masonry` is still not available -- Chrome 148 answers false to `grid-template-rows: masonry`, `display: masonry`, `item-pack` and `masonry-template-tracks` alike -- so the CSS-only way out is a plain grid with `align-items: start`, which was measured too: the order becomes left-to-right and the wall becomes **2696 px, 23% taller**, because a row is as tall as its tallest cell. That is the trade, and it is the packed Europeana look against the reading order. **It is not an accessibility defect**: the DOM carries the authors' order, so a screen reader and the tab ring already read it correctly and only the eye scans down a column                                                                                                                                                                                                                                                                                                                                                                              | when one of those is available                             |
| F21     | Cells no longer vary in width: a panorama takes the same room as a portrait                                                                                    | Same cause as F19. The height of each copy still carries the variety. **"Same cause as F19" was a slip for F20**, and the owner with it: the cause is `.wall` being CSS multi-column, where every column is one width, and it is F20 that unblocks it. **Measured over all 593 photographs**, the premise is nearly empty: `web_width / web_height` runs 0.40 (`trabajo-011`) to 2.90 (`educacion-020`), but p95 is 1.64 and p99 1.81, and only **7 photographs are wider than 1.8** -- 1.2% of the archive. The shape of the collection is two clusters, 148 portraits under 1.0 and 395 landscapes bunched between 1.3 and 1.8, and that variety is already carried by height exactly as the line above says. So a width rule would fire for seven scans. **Recommended dropped rather than deferred**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | the measurement says nobody                                |
| ~~F22~~ | ~~`theme-color` is `#26292c`, the logo's charcoal, while the page ground is `#1B1917`~~                                                                        | From the favicon design pass. On Android the address bar ends a shade cooler than the header, a visible seam. `background_color` in the manifest is correctly `#26292c`, since that is the icon's own ground. **Closed**: `THEME_COLOR` is `#1B1917` now, the same value as `--color-ground`, so the address bar and the page are one colour. The manifest's `theme_color` went with it, because it is the same seam on an installed app's title bar and the meta tag does not reach that; its `background_color` keeps the charcoal, which is the icon's ground and a different question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | done                                                       |
...
| ~~F25~~ | ~~Touch targets in the gallery's pagination are 16–19 px, under WCAG 2.2 SC 2.5.8's 24 px~~                                                                    | Found while sizing the photo page's own controls, which now clear it. Changing T6's shipped pagination was not this card's scope. **Closed**: every target in `Pagination` now carries the header nav's own box -- 24 x 44 for a number, the same 44 of height for the prev/next pair, which is wide enough on its own words. Measured before and after at 375 and 1280, and the row's "16-19 px" turned out to be one figure per kind: the numbers were 16 x 16 and the pair 20 px tall at 1280, 19 at 375, so the pair had to be fixed too and the `ponytail:` comment naming only the numbers was reading half of it. The row is 44 px at both widths, unchanged, and the far pages are still `display: none` on a phone -- which is what nearly broke it: `hidden` and `inline-flex` are both display utilities in one layer, so the shared box carrying `inline-flex` silently beat the `hidden` and every number showed at 375. Caught on the computed style, not the markup, so the box declares no display and each caller states its own                                                                                                                                                                                                                                                                                                                                                                     | done                                                       |
...
| ~~F30~~ | ~~`text-*` utilities do nothing on a `.t-credit` element~~                                                                                                     | Measured, not guessed: `.t-credit` is unlayered author CSS and Tailwind 4's utilities sit in `@layer utilities`, which loses whatever the specificity. T6 and T7 pair the two all over, and every one of those renders accent. **Closed, and the fix is a split rather than a move**: `color` alone goes into `@layer base`, for `.t-credit` and `.t-meta` -- swept across six public pages and those are the only two unlayered-colour classes the markup pairs with a colour utility, of the 21 that declare one. Measured before: `text-muted` on a `.t-credit` link rendered `#c9954e` where `#a2988a` was asked for, and about 19 `hover:text-*` per page never fired at all, which is why `.link` carries a `transition: color` for a colour that never changed. **The file could not move as a whole**: `.t-meta[aria-current]`, `.reveal > summary` over `blur-[9px]` and `body > header` over `z-20` all depend on outranking a utility, so layering everything would have broken three things to fix one. Verified in the CSSOM -- the two rules read `layer: base` and `.text-muted` and `.hover\:text-text:hover` read `layer: utilities` -- and on the computed colour: the muted links are muted, a bare `.t-credit` is still accent, and the pagination marker is still accent. The 12 `text-accent` on `.t-credit` on the home page were and remain no-ops: that is the colour the class already sets | done                                                       |
...
| F36     | The panel cannot move a photograph between sections                                                                                                            | The N:N relation is in the model and _What can be changed_ promises it, but T10's card scopes the fields, the flags, publishing, reordering and the restoration, and T11 shipped the categories themselves without it. Re-aimed at T17, which puts the control on the photograph's own edit screen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | T17                                                        |
...
| ~~F47~~ | ~~`npm run auth:smoke` has been failing on `main` since the panel screens got titles~~                                                                         | Not a regression and not a security defect: `tools/auth-smoke.ts:235` asserts that an anonymous `GET /admin/import` sends none of the screen, and `7e7c03f` gave that page the title `Importar desde Drive` -- which Next flushes in the head before `requireAdmin()`'s `redirect()` fires, so the string the assertion forbids is now in the 307's body. Checked against the production build: still 307 to `/admin/signin`, and the body carries the title and nothing else -- no folder names, no file list, no photo data. The assertion needs a marker that only appears in the rendered body. It aborts the run at step 10, so everything after it has not executed in a while. **Already closed when this row was written**: `3d02408` had replaced the assertion with a sentence of the screen's own prose, `Cada fotografía se importa de a una`, which is only ever rendered. Verified end to end against `npm run dev`: the run passes and reaches step 11, which reports 16 gated actions across 6 files                                                                                                                                                                                                                                                                                                                                                                                                  | done                                                       |
| F48     | On a cold instance the first request for a taken-down photograph can answer 404 instead of 410                                                                 | Seen while verifying T14 on `npm run start`: the proxy's blocking first read of `/api/gone` hit the 1.5 s timeout against a cold Neon compute and failed open, which is the direction T10 chose on purpose -- failing closed would 410 the whole archive. Every request after it answered 410 in all four languages. **It did not reproduce on Vercel**: the first request to `/foto/trabajo-056` on the live domain answered 410, encoded spelling included, so the window is narrower in production than on a laptop -- the data cache is warm and closer to Neon. Left as it is rather than tuned: raising the timeout trades it for a slower first paint on every photo page, and the real way out is the shared store F31 names.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Whoever minds; it is one constant                          |
| ~~F49~~ | ~~The footer shows the contact address but does not say that corrections and takedowns go to it~~                                                              | The residue of dropping `/sobre` in T14, and deliberately not code: the sentence belongs in `site_text.rights_notice`, which the panel edits without a deploy, so it is the authors' wording rather than the maintainer's. _Exposure, indexing and takedown on request_ promises the archive invites these requests, and until the sentence exists the invitation is an email address in a footer. **Dropped, and the premise is the part that was wrong**: it read the footer as the channel, and the footer is not the channel. Lautaro and Marcos do the work of getting the photographs and contacting the people, so a correction or a request to hide one reaches them directly, in a town where they know the families, long before it would reach an address in a footer. The address stays as the way in for somebody they do not know, which is what a `Contacto` label already says without a sentence under it. The promise was never the footer's: `ARCHITECTURE.md` was the thing claiming "the footer invites takedown requests on every page", and it has been corrected to what actually happens                                                                                                                                                                                                                                                                                                     | done                                                       |
...
| ~~F53~~ | ~~The panel builds a master's URL on the public domain, which the F35 rule now blocks~~                                                                        | `src/app/admin/photos/[slug]/page.tsx:89` falls back to `publicUrl(photo.masterKey)` for the preview when a photograph has no derivatives, and that address now answers 403. **Latent, not active**: 0 of 593 rows have a null `web_key`, so nothing reaches it today, and regeneration is unaffected because `readMaster` goes through the S3 API and not the custom domain. Found while verifying T14's own edge rule. The fix is to stop previewing a master over a public URL at all. **Closed**: the fallback is gone, and with it the `figcaption` branch that only that fallback could reach. A photograph with no derivatives now shows no preview, which is F39's to fill and not a broken picture                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | done                                                       |
| ~~F54~~ | ~~`npm run lint` reports hundreds of errors from `.claude/worktrees`~~                                                                                         | Measured on `main` while verifying T14: 357 errors and 7,285 warnings, every one of them from a copy of the repository inside `.claude/worktrees/…` that the harness creates when a background task is started. Over `src tools next.config.ts` the same run is clean but for the one pre-existing `<img>` warning. Nothing in the project is wrong, but anybody running `npm run lint` after using a worktree sees a wall of red and has to work out that none of it is theirs. One `ignores` entry for `.claude/` in `eslint.config.mjs` closes it. **Already closed when this row was written**: `51b2aa6` carries the entry, as `.claude/worktrees/**` inside the existing `globalIgnores`. Verified: `npx eslint .` over the whole repository reports 0 errors and the one pre-existing `<img>` warning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | done                                                       |
| ~~F55~~ | ~~A build can pre-render from a list of slugs that predates the database change~~                                                                              | **Found in T16 and it is not the Videoteca's**: `generateStaticParams` reads through `unstable_cache`, whose entries live in `.next/cache` and survive between a panel write and the next build. Measured -- three videos created from the panel, then a build, and the segment pre-rendered **zero** of its twelve pages, because `listVideoSlugs` answered from the entry written when the table was empty. `rm -rf .next/cache` produced all sixteen. It applies to `listPhotoSlugs` and `countSectionPhotos` identically, which is why nobody has seen it: what it costs is a pre-render, not a page, since `dynamicParams = true` renders the missing one on demand. Vercel restores the build cache between deploys, so it is not only a laptop problem. **Closed**: `listPhotoSlugs`, `countSectionPhotos` and `listVideoSlugs` no longer go through `unstable_cache`. Each has exactly one caller and it is a `generateStaticParams`, so the persistent entry bought nothing and cost correctness. Uncached they are one indexed query per build. **What it does not close, now F58**: the same segments also read `listSections` and `listCategoryOrder`, which stay cached because request paths share them                                                                                                                                                                                                 | done                                                       |
