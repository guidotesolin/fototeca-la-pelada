 ...
 * Two of these fields end up in an `<iframe src>` and three in an `href`, and
 * they are typed by a person: an administrator is trusted with the archive, which
 * is not the same as being trusted to have pasted a URL a naive parser reads the
 * way it looks. So a value is put through `mapEmbedUrl`/`externalUrl` **before it
 * is stored**, and what is stored is the guard's own normalised output -- not the
 * typed string. The public side runs the same guards again on the way out, which
 * is not redundant: the rows predate this screen and a database is not only
...
function clean(form: FormData, key: string, kind: keyof typeof LIMITS): string | null {
  ...
  switch (kind) {
    case 'map': {
      const url = mapEmbedUrl(value)
      if (!url) throw new Invalid('url-mapa')
      return url
    }
    case 'link': {
...
export async function saveSiteText(form: FormData) {
  ...
  const result = await outcome('site-text', 'textos', async () => {
    // Everything is validated before anything is written: a rejected map URL must
    // not leave the other eleven fields saved and the screen reporting a failure.
    ...
    await db.transaction(async (tx) => {
      if (present.length) {
        await tx
          ...
          // twelve keys are one statement instead of twelve round trips.
          .onConflictDoUpdate({
